...
library(dplyr)
library(ggplot2)
setwd("/Users/jamesledoux/Documents/Research/Thesis/Data/cincinnati")


#cleaned and merged DataFrame
df = fread("cincinnati_crime.csv", data.table=FALSE)
...
df$Block = with(df, paste0(BlockBegin, BlockEnd, StreetName))

#get years 
df$Year = sub("^[^/]*", "", df$OccurredOn)
df$Year = sub("/", "", df$OccurredOn)
df$Year = sub("^[^/]*", "", df$Year)
df$Year = sub("/", "", df$Year)
...
#sort(table(df$Offense), decreasing = TRUE)
violent_crime = c('ASSAULT -(Simple Assault)', 'DOMESTIC VIOLENCE -(Simple Assault)', 'AGGRAVATED ROBBERY',
                  'FELONIOUS ASSAULT -(Aggravated Assault)', 'Assault -(Simple Assault)', 
                  'ROBBERY', 'Domestic Violence -(Simple Assault)', 'RAPE -(Forcible Rape)', 
                  'Aggravated Robbery', 'Felonious Assault -(Aggravated Assault)', 'Robbery',
                  'ASSAULT -(Aggravated Assault)', 'MURDER -(Willful Killing)', 'DOMESTIC VIOLENCE -(Aggravated Assault)',
                  'Rape -(Forcible Rape)', 'FELONIOUS ASSAULT -(Simple Assault)', 'ENDANGERING CHILDREN -(Simple Assault)',
                  'Aggravated Robbery _ deadly weapon on or about suspect, control, display, brandish, indicate possession, or use weapon-',
                  'RAPE -(Simple Assault)', 'Assault -(Aggravated Assault)', 'SEXUAL BATTERY -(Forcible Rape)', 'Felonious Assault -(Simple Assault)',
                  'Domestic Violence _ knowingly cause physical harm (Simple Assault)', 'AGGRAVATED ASSAULT', 'RAPE -(Forcible Sodomy)',
                  'Murder -(Willful Killing)', 'UNLAWFUL SEXUAL CONDUCT WITH A MINOR -(Simple Assault)', 'Domestic Violence -(Aggravated Assault)',
                  'Felonious Assault _ victim seriously harmed (Aggravated Assault)', 'AGGRAVATED MURDER -(Willful Killing)',
                  'AGGRAVATED VEHICULAR HOMICIDE VEHICULAR HOMICIDE VEHICULAR MANSLAUGHTER', 'Assault _ knowingly harm victim (Simple Assault)',
                  'AGGRAVATED VEHICULAR ASSAULT VEHICULAR ASSAULT', 'Robbery _ Use or threaten the immediate use of force against another-')

df = df[df$Offense %in% violent_crime,]
#get total number of street segmets in the city
#transportation.shapefile = readOGR(dsn="sacramento_streets.geojson", layer="OGRGeoJSON", p4s="+proj=tmerc +ellps=WGS84")

streets = read.csv("cincinnati_streets.csv")
num_segments = length(unique(streets$STRSEGID))
streets = streets[!duplicated(streets$STRSEGID),]
seglength = mean(streets$LENGTH)
num_segments = 13550
cat("num segments: ", num_segments)
cat("average segment length: ", seglength, " ft.")
...
#sort(table(df$Offense), decreasing=T)
...
for(year in years_in_data){
  if(year>7){  #pre-2009 data looks sketchy in this
