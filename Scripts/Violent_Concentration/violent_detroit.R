#####################################################################
# Detroit
# unfinished. need to get the year/date right.
#####################################################################

setwd("/Users/jamesledoux/Documents/Research/Thesis/Data/Detroit")
...
df = fread("detroit.csv", data.table=FALSE)
...
# intersections defined by [/, &, 'and', and sometimes just 00 [stnames]]. The last of these I might not be able to filter out
before = nrow(df)
...
df = df[!grepl('@', df$ADDRESS),] #get rid of intersections
after = nrow(df)
pct_at_intersections = (before-after)/before
cat("pct of crime at intersections: ", pct_at_intersections)

num_segments = 34000 #hardcoding for now... can't find a centerline file?
...
violent_crimes = c("ASSAULT", "ROBBERY", "AGGRAVATED ASSAULT", "HOMICIDE", "JUSTIFIABLE HOMICIDE",
                   "NEGLIGENT HOMICIDE") #no sex crimes?
