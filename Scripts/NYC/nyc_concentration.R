##
# 5 prepared, 10ish remaining.. Try to have 10 publishable city datapoints.
# TODO: Detroit, NYC, Boston, Oakland (?), Philly, Pitt, SAC, Baton Rouge, Austin, Cincinnati

#also see how the results look when limiting them to violent crime (ie the violent 7 from UCR)

# other desired features:
# proximity to sports stadium
# proximity to drug rehab center
# to subway / bus stop 
# to halfway house 
# to bar / pub / licensed liquor seller 
# see what else we can get for economic details (income and age could be cool if we can get that micro)

# also: get outputs for all cities in my data in some common space (an image folder, a folder of results data frames, . . .)
# it will save tons of time to have their concentration figures all in one place w/o having to re run the data each time
#   also: refine more as you record these numbers. give the non seattle citeis the due diligence they deserve
#   record this info for both all crime and the violent seven


#take these same measures for all years combined for each dataset
#do this on a shared set of crimes (the violent 7)
#store this data in a table, since this is what will go in the summary stats section of the paper

# then: join on some RHS vars and try simple OLS

# then: trajectory analysis. this goes after ols since it's easy to swap this in as the RHS later on.
##
...
# currently stalled b/c data has points, not segments
# see: https://cran.r-project.org/web/packages/stplanr/stplanr.pdf and https://www.nceas.ucsb.edu/scicomp/usecases/AssignClosestPointsToPoints for potential workarounds
#####################################################################

setwd("/Users/jamesledoux/Documents/Research/Thesis/Data/NYC")
...
#df = fread("NYPD_Complaint_Data_Historic.csv", data.table=FALSE)
df = fread("../nyc_felonies.csv", data.table=FALSE)
